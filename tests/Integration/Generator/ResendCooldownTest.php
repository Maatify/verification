...
final class ResendCooldownTest extends DatabaseTestCase
{
    ...
    public function test_second_generate_within_cooldown_fails(): void
    {
        $this->pdo->exec("
            INSERT INTO verification_codes
            (identity_type, identity_id, purpose, code_hash, status, attempts, max_attempts, expires_at, created_at)
            VALUES
            ('user', 'user123', 'email_verification', 'hash1', 'active', 0, 3, '2024-01-01 12:10:00', '2024-01-01 12:00:00')
        ");

        $this->clock = $this->createMock(ClockInterface::class);
        $this->clock->method('getTimezone')->willReturn(new \DateTimeZone('UTC'));
        $this->clock->method('now')->willReturn(new DateTimeImmutable('2024-01-01 12:00:30', new \DateTimeZone('UTC')));
        ...
        $this->expectException(VerificationResendCooldownException::class);
